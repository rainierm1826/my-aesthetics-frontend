...
//   // Helper: pick a primary service to edit (first service)
//   const primaryService = appointment.services?.[0];

//   // Get appointment date from the primary service start_time or fallback to today
//   const getAppointmentDate = (): string => {
//     const src = primaryService?.start_time || appointment.created_at;
//     if (src) {
//       const d = new Date(typeof src === "string" ? src.replace(" ", "T") : src);
//       if (!isNaN(d.getTime())) return d.toLocaleDateString("en-PH");
//     }
...
//     (primaryService?.start_time ? String(primaryService.start_time).slice(11, 16) : ""),
//     (primaryService?.duration_snapshot as number) || 60
//   );
...
//       service_id: primaryService?.service_id ? String(primaryService.service_id) : "",
//       aesthetician_id: primaryService?.aesthetician_id ? String(primaryService.aesthetician_id) : "",
...
//         payload.walk_in_id = String(appointment.walk_in_id);
//         payload.is_walk_in = true;
//       } else if (appointment.user_id) {
//         payload.user_id = String(appointment.user_id);
